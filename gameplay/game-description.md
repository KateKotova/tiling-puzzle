...
У пользователя будут жизни. За то, что пользователь включает подсказку-глазик, жизни снимаются.
Таким образом, количество включений лампочки-подсказки ограничено количеством жизней.
...
4. Динамик - включить/выключить звук.
В игре будет весёлая музыка.
Думаю, по умолчанию музыка будет включённой.
Регулятор громкости можно поместить в настройках.

5. Рублик - кнопка для донатов.
Надо сделать её небольшой, чтоб она не отвлекала и предусмотреть возможность, когда её может не быть.
...
У пользователя будет полоса жизней.
я думаю представить жизни в виде полоски прогресса, где указывается количество жизней.
Количество жизней - это количество подсказок, которое пользователь может получить при данном раскладе.

Неиспользуемые на текущем уровне жизни можно переносить на следующий уровень, где тоже используются жизни.
Можно переносить один к одному, а можно в другой пропорции, например, вполовину меньше, а то у прозорливых пользователей к конце угры может оказаться слишком много жизней.

Но можно и не изменять пропорцию сохранения жизней, а оставить, как есть, и в уровнях на время.
В уровнях на время жизни могут превращаться в дополнительное время для сборки, и это было бы интересно.
...
Можно использовать жизни как валюту и получать за них скрытые уровни, за которые будут полагаться особенные достижения.
...
Если учитывать жизни как дополнительное время, то после завершения основного времени пойдёт дополнительное время.
Элементы управления при этом можно отметить другим цветом, например, красным. Время пойдёт, а жизни при этом начнут таять.
...
*Как вы думаете, может сделать время в виде полосы прогресса?*
*Тогда, если жизни - тоже полоса прогресса, то хорошо ли будет смотреться, когда полос прогресса две?*
*Как вы относитесь к кнопке паузы? Мне кажется, она нужна, а как вы думаете?*
*Что вы думаете про тающие жизни в качестве дополнительного времени?*
...
Для частного случая квадратных экранов будет применяться вертикальное расположение.

### Вертикальный экран
1. В самом верху:
    - полоса жизни (подсказок);
    - таймер и кнопка паузы - для уровней на время;
    - просто счётчик времени - для всех уровней, кроме уровней на время;
...
2. Зачем вверху, но чуть пониже:
    ...
    - динамик - включить/отключить выбранные звуки;
    - рублик - для донатов.
...
2. Зачем слева, но чуть правее:
    ...
    - рублик - для донатов.
3. В средине: картинка с ячейками для сборки мозаики.
4. Справа: лента с кнопками прокрутки.
...
*Что делать, если пользователь решил пройти уровень сложностью ниже ради жизней?*
...
В начале будут онбординговые уровни. Это уровни, в которых будет ограниченный функционал для изменений и перепрохождений, потому что пользователь только-только осваивает функции. Но в этих уровнях будут свои достижения и заработанные жизни, которые будут учитываться, как обычно.
...
я предлагаю сделать так. Выбрать 2 цвета, например, синий и малиновый (какие-то контрастные), и сделать такую растяжку цвета на каком-то заданном интервале. Интервал будет представлять сложности от минимальной до максимальной на заданном уровне. Промежуточные сложности в достижениях будут иметь промежуточные цвета, например, если сложность совсем средняя, то она будет фиолетовой.
...
- будут даны жизни на расходование подсказок.
...
Будет дана следующая **информация**:
- пользователю будут даны жизни, которые он может расходовать на подсказки-глазики и на время;
...
- будет объяснён механизм применения жизней в контексте времени;
